...
import { GetSessionStateByIdService } from "./getSessionStateById.service"
...
export class RefreshSessionService {
  ...
  public async execute(
    ...
    const oldState =
      await this.getSessionStateByIdService.execute(sid)
    if (oldState.revoked) {
    ...
    await this.revokeSessionStateService.execute(oldState.id)

    const newState =
      await this.findOrCreateSessionStateService.execute(
        sub,
        userAgent,
      )
